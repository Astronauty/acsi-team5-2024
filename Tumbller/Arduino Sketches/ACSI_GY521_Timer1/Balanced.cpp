...
void Balanced::PD_VerticalRing()
{
  ...
  float angleBias = 0;
  float gyroBias = -0.98 ;
  float contribution_p = kp_balance * (angle - angleBias);
  float contribution_d = kd_balance * (kalmanfilter.Gyro_x - gyroBias);
...
bool Mpu6050::readIMU()
{
  ...
  // Log the 5 most recent values of accRoll and gx
  accRollValues[valueIndex] = accRoll;
  gxValues[valueIndex] = gx;
  ...
  float avgGx = 0;
  for (int i = 0; i < 5; i++) {
    avgAccRoll += accRollValues[i];
    avgGx += gxValues[i];
  }
  avgAccRoll /= 5;
  avgGx /= 5;

  // Pass the average values to Kalman_Filter_New
  kalmanfilter.Kalman_Filter_New(avgAccRoll, avgGx, dt, Q_angle, Q_gyro, R_angle, C_0);
...
{
  accRoll = atan2(ay, az) * 180 / 3.14159;
  accPitch = atan2(-ax, sqrt(ay * ay + az * az)) * 180 / 3.14159;
  ...
  complementaryRoll = 0.11 * complementaryRoll + 0.89 * accRoll;
  complementaryPitch = 0.11 * complementaryPitch + 0.89 * accPitch;
