...
void KalmanFilter::Kalman_Filter(double angle_m, double gyro_m,float dt,float Q_angle,float Q_gyro,float R_angle,float C_0) {
  ...
  Serial.println("Kalman_Filter");
  angle += (gyro_m - q_bias) * dt; // gyro_m is in deg/sec
...
void KalmanFilter::Kalman_Filter_New(double angle_m, double gyro_m, float dt,float Q_angle,float Q_gyro,float R_angle,float C_0) {
  // float check_time = micros(); // takes 0.16 ms to run
  angle = angle_m; // gyro is already handled in KalmanFilter::Angle
  // Gyro_x = gyro_m;
  Gyro_x = (gyro_m - 12.1) / 13; // subtract drift, divide
}
...
void KalmanFilter::Angle(float ax, float ay, float az, float gx, float gy, float gz, float dt, float Q_angle, float Q_gyro, float R_angle, float C_0, float K1) {
  ...
  // Gyro_x = -(gx - 10) / 131;
  // float drift_gryo = 128.1;
  // float scale_gyro = 131;
  // Gyro_x = (gx - drift_gryo) / scale_gyro; // subtract drift, divide

  // Kalman_Filter(double angle_m, double gyro_m,float dt,float Q_angle,float Q_gyro,float R_angle,float C_0);
...
void KalmanFilter::Angle_New(float roll, float omega, float az, float gx, float gy, float gz, float dt, float Q_angle, float Q_gyro, float R_angle, float C_0, float K1) {
  // float Angle = atan2(ay , az) * 57.3; // atan2(ay, az) on standard Nano 
  // Gyro_x = (gx - 128.1) / 131; // subtract drift, divide
  // Kalman_Filter_New(Angle, Gyro_x, dt, Q_angle, Q_gyro, R_angle, C_0); // check 1
  Kalman_Filter_New(roll, omega, dt, Q_angle, Q_gyro, R_angle, C_0); // check 1
  // angle = Angle; // OH test shows it jumping from 0 to +-6.34; the culprit is precision not MEMS inertia or KF lag
  Yiorderfilter(roll, Gyro_x, dt, K1); // check 2
  // Yiorderfilter(angle, Gyro_x, dt, K1); // check 3 - not a good idea
  Gyro_z = -gz / 131;
}
