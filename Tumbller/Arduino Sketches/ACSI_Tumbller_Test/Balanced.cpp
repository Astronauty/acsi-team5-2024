...
Balanced::Balanced(Mpu6050& mpu):mpu6050(mpu)
{
  ...
  kp_balance = 20, kd_balance = 0.035; //.75;  
...
void Balanced::PD_VerticalRing()
{
  ...
  float contribution_d = kd_balance * (kalmanfilter.Gyro_x - (0.98)); // change this to steady state value
  Serial.print("P: ");
...
void Mpu6050::doCalculations() // replaces DataProcessing
{
  ...
  complementaryRoll = (1-weight) * complementaryRoll + weight * accRoll;
  complementaryPitch = (1-weight) * complementaryPitch + weight * accPitch;
}