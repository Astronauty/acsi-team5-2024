...
void MPU6050::initialize() {
  ...
  setClockSource(MPU6050_CLOCK_PLL_XGYRO); // dies here for w/e reason on SAMD
  Serial.println("Clock source set");
  setFullScaleGyroRange(MPU6050_GYRO_FS_250);
  Serial.println("Gyro range set");
  setFullScaleAccelRange(MPU6050_ACCEL_FS_2);
  Serial.println("Accelerometer range set");
...
void MPU6050::setClockSource(uint8_t source) {
  Serial.println("setClockSource running");
  Serial.println(devAddr);
  Serial.println(MPU6050_RA_PWR_MGMT_1);
  Serial.println(MPU6050_PWR1_CLKSEL_BIT);
  Serial.println(MPU6050_PWR1_CLKSEL_LENGTH);
